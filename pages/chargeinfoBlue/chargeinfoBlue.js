    ...
    gofororder() {
        ...
        if (
            ...
            wx.requestSubscribeMessage({
                ...
                success(res) {
                    HTTP({
                        url: "app/ctl/getChargingCommandV2",
                        methods: "get",
                        data: {
                            chargingPortId: that.data.selectChargPort,
                            chargingTime: that.data.selectTime,
                            cabinetDeviceId: that.data.cabinetinfo.cabinetDeviceId
                        }
                    }).then(res => {
                        wx.showLoading({
                            title: "请稍后",
                        ...
                        if (res.code == 200) {
                            let data = res.data;
                            wx.setStorageSync("ordersearch", res.data.data)
                            bluetooth.sentOrder(data.data, "cmdChargingOrder");
                            app.globalData.emitter.once(
                                "cmdChargingOrder",
                                (response) => {
                                    if (response) {
                                        HTTP({
                                            url: "app/ctl/blueStartCharging",
                                            methods: 'post',
                                            data: {
                                            ...
                                            wx.redirectTo({
                                                url:
                                                    "/pages/chargeServingBlue/chargeServingBlue?charingOrderNo=" +
                                                    data.orderNo
                                            });
                                            //数据发送结束
                                            that.setData({
